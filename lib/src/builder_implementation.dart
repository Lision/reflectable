  ...
  Future<String> generateCode() async {
    ...
    for (_ReflectorDomain reflector in reflectors) {
      ...
      for (LibraryElement library in libraries) {
        ...
        // 过滤非待分析 package /lib/src/ 目录下的文件
        String libiaryPath = library.source.uri.path;
        if (!libiaryPath.startsWith(path.basename(path.current)) ||
            !libiaryPath.contains('/lib/src')) {
          continue;
        }
...
class _ReflectorDomain {
  ...
  Future<String> _generateCode(ReflectionWorld world,
    ...
    String code = privateClassMirrorsList.join("\n");

    // process code
    List<String> resultStrings = <String>[];
    String markString = "// This file is auto generated by the proxy package.";
    List<String> importStrings = <String>[];
    if (code.isNotEmpty) {
      importStrings.add("import 'package:base_proxy/type_base_proxy.dart';");
    }
