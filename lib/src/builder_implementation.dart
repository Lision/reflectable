...
class _ReflectorDomain {
  ...
  Future<String> _generateCode(ReflectionWorld world,
    ...
    if (_capabilities._impliesTypes || _capabilities._impliesInstanceInvoke) {
      for (_ClassDomain classDomain in (await classes).domains) {
        // 过滤非本文件的目标
        if (classDomain._classElement.library != _world.entryPointLibrary) {
          continue;
        }
...
// """
    ...
    String mark = "// This file is auto generated by the proxy package.";
    List<String> imports = <String>[];
    String code = await world.generateCode();
    if (code.isNotEmpty) {
      imports.add("import 'package:hera/src/mirror/proxy_type_base.dart';");
    }
...
${results.join('\n\n')}
    ...
    if (_formatted) {
      ...
      resultCode = formatter.format(resultCode);
    }
    return resultCode;
  }
